...
def train_epoch(model, loader, optimizer, loss_fn, device, max_batches=None):
    ...
    for batch_idx, (x, _) in enumerate(tqdm(loader, leave=False)):
        ...
        loss = loss_fn(reconstruction, target).mean()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def collect_scores(model, loader, loss_fn, device, max_batches=None):
    model.eval()
    scores, labels = [], []
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(tqdm(loader, leave=False)):
            ...
            target = denormalize_imagenet(x)
            reconstruction = model(x)
            per_image = loss_fn(reconstruction, target).mean(dim=(1, 2, 3))
...
def main():
    ...
    ap.add_argument("--image_size", type=int, default=64, choices=[28, 64, 128, 224])
    ...
    ap.add_argument("--checkpoint", default="checkpoints/autoencoder_chestmnist.pt")
    ...
    artifacts_dir = Path("outputs") / "chestmnist_anomaly" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    run_name = "smoke_autoencoder" if is_smoke_test else "conv_autoencoder_normal_only"
    with mlflow.start_run(run_name=run_name):
        ...
        for epoch in range(args.epochs):
            train_loss = train_epoch(model, train_loader, optimizer, loss_fn, device, args.max_train_batches)
            normal_scores, _, _, _ = collect_scores(
                model, val_normal_loader, loss_fn, device, args.max_eval_batches
            )
            val_loss = float(normal_scores.mean())
            mlflow.log_metric("train_loss", train_loss, step=epoch)
            mlflow.log_metric("val_normal_reconstruction_loss", val_loss, step=epoch)
            print(f"epoch={epoch + 1} train_loss={train_loss:.6f} val_normal_loss={val_loss:.6f}")
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                torch.save({"model": model.state_dict()}, checkpoint)

        model.load_state_dict(torch.load(checkpoint, map_location=device)["model"])
        normal_scores, _, _, _ = collect_scores(model, val_normal_loader, loss_fn, device, args.max_eval_batches)
        ...
        val_scores, val_labels, val_images, val_reconstructions = collect_scores(
            model, val_loader, loss_fn, device, args.max_eval_batches
        )
        test_scores, test_labels, _, _ = collect_scores(
            model, test_loader, loss_fn, device, args.max_eval_batches
        )
